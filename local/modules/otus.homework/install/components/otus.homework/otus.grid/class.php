...
class OtusGridComponent extends CBitrixComponent
{
    ...
    {
        $logFile = $_SERVER['DOCUMENT_ROOT'] . '/lazyload_log.txt';
        file_put_contents($logFile, "[class.php] Параметры: " . print_r($this->arParams, true) . "\n", FILE_APPEND);
        ...
        if (!\Bitrix\Main\Loader::includeModule('iblock')) {
            ...
            file_put_contents($logFile, "[class.php] Модуль Инфоблоки успешно подключен\n", FILE_APPEND);
        }
        ...
        while ($item = $res->GetNext()) {
            file_put_contents($logFile, "[class.php] Элемент: " . print_r($item, true) . "\n", FILE_APPEND);
            $this->arResult['ITEMS'][] = $item;
        }

        // Логируем количество найденных элементов
        file_put_contents($logFile, "[class.php] Найдено элементов: " . count($this->arResult['ITEMS']) . "\n", FILE_APPEND);

        // Если данных нет, выводим сообщение в лог
        if (empty($this->arResult['ITEMS'])) {
            file_put_contents($logFile, "[class.php] Внимание: Данных нет\n", FILE_APPEND);
        }

        // Подключаем шаблон
        file_put_contents($logFile, "[class.php] Подключаем шаблон\n", FILE_APPEND);
